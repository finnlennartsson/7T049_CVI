...
Options:
  -dwi				Preprocessed dMRI data serie (format: .mif.gz) (default: derivatives/dMRI/sub-sID/dwi/dwi_preproc_norm.mif.gz)
  ...
  -5TT				5TT in dMRI space (format: .mif.gz) (default: derivatives/dMRI/sub-sID/dwi/act/5TT_coreg.mif.gz)
...
act5tt=$datadir/dwi/act/5TT_coreg.mif.gz
...
if [[ $response = msmt_5tt ]]; then
    ...
    echo "Estimating response function use $response method"
    dwi2response msmt_5tt -force -voxels $csddir/${response}_sf.mif -fa 0.15 $dwi.mif.gz act/$act5tt.mif.gz $csddir/${response}_wm.txt $csddir/${response}_gm.txt $csddir/${response}_csf.txt
